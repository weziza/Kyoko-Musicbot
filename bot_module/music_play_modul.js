...
var URLArray=[ // zufall url ergenzung wenn die suche fehlschlägt
    ...
    "Mgfe5tIwOj0"]
//------------------------------
exports.get_song = function (memberchannel, message, RandomColor,bot_MessChannel) {
...
exports.play_song = function (memberchannel, message, RandomColor,bot_MessChannel,url) {
    ...
    if (MaxQueue==MinQueue){ // ist das max der song aufnahme erreicht dann....
        ...
        ytdl.getInfo(url = String(url), (error, videoInfo) => { // url ausgabe information
            ...
            TrackTitel.push(videoInfo.title +" : "+ time.toFixed(2) + " min");
            TrackTitel_Array = TrackTitel.map((TrackTitel, x) => ((x + 1) + ': ' + TrackTitel)).join('\n');            
            //---------------------------------------
            MinQueue++; // Song counder ++
            //---------------------------------------
...
exports.queue_function =  function(message,bot_MessChannel) {
    ...
    //-----------------------------
    if (TrackTitel_Array.length<1) {
        bmess.ambedMessage("Kein Song", "in der Warteschlange", bot_MessChannel, RandomColor, BotName, botAuthorImage, NoQueueListe);
    }
    else {
        bmess.ambedMessage("- in der Warteschlange  :", TrackTitel_Array, bot_MessChannel, RandomColor, BotName, botAuthorImage, QueueListe);
    }
    return;
};
//---------------------------------------
...
exports.clean_queue = function(message,bot_MessChannel){
    ...
    var RandomColor = '0x'+(0x1000000+(Math.random())*0xffffff).toString(16).substr(sub,6)
    //-----------------------------
    if(TrackTitel_Array.length>0){
        bot_defaultVolume_option=true; 
        MinQueue=0;  // Queue auf null setzen                                 
        Warteschlange_Array=[];  // warteschlange leeren   
        TrackTitel_Array=[]; // Song titel leeren leeren
        TrackTitel = [];
